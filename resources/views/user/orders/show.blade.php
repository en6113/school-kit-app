                ...
                <li class="py-4 flex justify-between">
                    <div>
                        ...
                        <p class="text-sm text-gray-500">
                            単価: ¥{{ number_format($product->pivot->price_at_purchase) }} / 数量:
                            {{ $product->pivot->quantity }}
                        </p>
                    </div>
                    <p class="font-bold">
                        ¥{{ number_format($product->pivot->price_at_purchase * $product->pivot->quantity) }}</p>
                </li>
            @endforeach
        </ul>
    ...
    @if($order->status == 1 && $order->user_id === auth()->id())
        <div class="flex justify-end">
            <form action="{{ route('orders.cancel', $order->id) }}" method="POST"
                ...
                @csrf
                @method('PATCH') {{-- cancelメソッドがPATCHまたはPOST想定なら合わせる --}}
